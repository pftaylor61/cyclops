...
if (!class_exists('\\BootstrapBasic4\\Bsb4Design')) {
    ...
    class Bsb4Design
    {
        ...
        {
            $output = sprintf('<span class="categories-icon fas fa-th-list" title="' . __('Posted in', 'cyclops') . '"></span> %1$s', $categories_list);
        ...
        {
            $output = __('Continue reading <span class="meta-nav">&rarr;</span>', 'cyclops');
            ...
            if ('pingback' == $comment->comment_type || 'trackback' == $comment->comment_type) { 
                ...
                echo '<div class="comment-body media">';
                    echo '<div class="media-body">';
                        _e('Pingback:', 'cyclops');
                        comment_author_link(); 
                        edit_comment_link(__('Edit', 'cyclops'), '<span class="edit-link">', '</span>');
                    echo '</div>';
                echo '</div>';
            } else {
                echo '<li id="comment-';
                ...
                echo '" class="comment-body media">';
                    ...
                    echo '<div class="comment-content media-body">';
                        echo '<div class="comment-author vcard">';
                            ...
                            printf(_x('%1$s at %2$s', '1: date, 2: time', 'cyclops'), get_comment_date(), get_comment_time());
                            echo '</time>';
                            echo '</a>';
                            // end date-time
                            ...
                            edit_comment_link('<span class="far fa-edit "></span>' . __('Edit', 'cyclops'), '<span class="edit-link">', '</span>');
                            ...
                            if ('0' == $comment->comment_approved) {
                                echo '<div class="comment-awaiting-moderation text-warning"> <span class="fas fa-info-circle"></span> ';
                                    _e('Your comment is awaiting moderation.', 'cyclops');
                                echo '</div>';
                            } //endif;

                            // comment author says
                            /* translators: $s: Comment author name with link. */
                            printf(__('%s <span class="says">says:</span>', 'cyclops'), sprintf('<cite class="fn">%s</cite>', get_comment_author_link()));
                        ...
                        comment_reply_link(array_merge($args, array(
                            ...
                            'reply_text' => '<span class="fas fa-reply"></span> ' . __('Reply', 'cyclops'),
                            'login_text' => '<span class="fas fa-reply"></span> ' . __('Log in to Reply', 'cyclops')
                        )));
                        // end reply link
        ...
        public function editPostLink($return = false)
        {
            ...
            if ($edit_post_link != null) {
                $edit_btn = '<a class="post-edit-link btn btn-light btn-sm" href="'.$edit_post_link.'" title="' . __('Edit', 'cyclops') . '" role="button"><i class="edit-post-icon far fa-edit" title="' . __('Edit', 'cyclops') . '"></i></a>';
        ...
        public function pagination($pagination_align_class = 'justify-content-center', $return = false)
        {
            ...
            if (is_array($pagination_array) && !empty($pagination_array)) {
                $output .= '<nav class="pagination-nav-container" aria-label="'.esc_attr__('Page navigation', 'cyclops').'">';
        ...
        public function postOn($return = false)
        {
            ...
            $output = sprintf(__('<span class="posted-on">Posted on %1$s</span><span class="byline"> by %2$s</span>', 'cyclops'),
                ...
                sprintf('<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span>',
                    ...
                    esc_attr(sprintf(__('View all posts by %s', 'cyclops'), get_the_author())),
        ...
        {
            $output = sprintf('<span class="tags-icon fas fa-tags" title="' . __('Tagged', 'cyclops') . '"></span> %1$s', $tags_list);
